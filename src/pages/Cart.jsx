...
const Cart = () => {
  ...
  const { product, navigate, cartCount, totalCartAmmount, cartItems, removeFromCart, updateCartItem } = useContext(AppContext);
  const [cartArray, setCartArray] = useState([]);
  const [address, setAddress] = useState(dummyAddress);
  const [selectedAddress, setSelectedAddress] = useState(dummyAddress[0]);
  const [paymentOption, setPaymentOption] = useState('COD');

  // Correct Cart Items extraction
  useEffect(() => {
    ...
    for (let productId in cartItems) {
      ...
      const productData = product.find(item => item._id === productId); // Corrected _id
      ...
      if (productData) {
        newCartArray.push({
          ...
          totalPrice: productData.offerPrice * cartItems[productId],
        });
      }
    }
    console.log("Final newCartArray:", newCartArray);
  ...
  return cartArray.length ? (
      ...
      <div className='flex-1 max-w-4xl'>
        ...
        {cartArray.map((product, index) => (
          <div key={index} className="grid grid-cols-[2fr_1fr_1fr] text-gray-500 items-center text-sm md:text-base font-medium pt-3">
            ...
            <p className="text-center">${product.totalPrice}</p>
            <button
      ...
      <div className="max-w-[360px] w-full bg-gray-100/40 p-5 max-md:mt-16 border border-gray-300/70">
        ...
        <div className="text-gray-500 mt-4 space-y-2">
          <p className="flex justify-between">
            <span>Price</span><span>${totalCartAmmount()}</span>
          </p>
          <p className="flex justify-between">
            <span>Shipping Fee</span><span className="text-green-600">Free</span>
          </p>
          <p className="flex justify-between">
            <span>Tax (2%)</span><span>${(totalCartAmmount() * 0.02).toFixed(2)}</span>
          </p>
          <p className="flex justify-between text-lg font-medium mt-3">
            <span>Total Amount:</span><span>${(totalCartAmmount() * 1.02).toFixed(2)}</span>
          </p>
        </div>
    ...
    <h2 className="text-center py-20 text-gray-500">Your cart is empty</h2>
  );
};
